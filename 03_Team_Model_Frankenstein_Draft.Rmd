...
Home Credit: Predictive Analysis to Identify Loan Repayment-Ready Customers

### Business Problem Statement

Home Credit lends to individuals with limited credit history. To grow responsibly, we need to estimate each applicant’s likelihood of default and use that risk score to guide approve/review decisions. A better risk signal lets us approve more good loans while avoiding bad ones.

### Benefit

The benefits of accurately predicting target customers are as follows:

1. Approve more qualified customers without increasing loss rates.
2. Reduce exposure to defaults and portfolio volatility.
3. Expand access to fair credit for underserved but creditworthy applicants.

### Analytics Approach

We will use the Kaggle Home Credit dataset to build supervised learning models that output a risk score (0–1). Models will be evaluated primarily by AUC-ROC (ranking quality across all thresholds), with PR-AUC as a secondary metric due to class imbalance. We will also report thresholded metrics at a simple operating policy (for example, flag the top 10% highest-risk applicants) and show precision, recall, and a confusion matrix at that cut.

Data preparation steps:

* Drop ID columns.
* Impute missing numeric values with the training median and missing categorical values with the training mode.
* Collapse rare categories into “Other,” then one-hot encode categorical variables.
* Remove zero-variance columns.
* Fit all preprocessing on the training split only, then apply to validation/holdout/test to prevent leakage.
* Use stratified resampling to preserve the default rate.
...
* Gradient boosting (expected strongest on tabular data).

The best model by cross-validated AUC will be selected and paired with the chosen operating policy. We are not using a “95% repayment/accuracy” target; with an imbalanced base rate, accuracy is misleading. We optimize AUC first, then choose a threshold that balances business trade-offs.

### Scope and Deliverables

This project will focus on the Kaggle Home Credit datasets only. Deliverables include a reproducible analysis notebook, a concise slide presentation, and a short summary of the recommended operating policy, key features, and a basic monitoring plan.

### Project Details

This project will be executed by Whitney Bullock, Thomas Beck, and Caleb Call (University of Utah MSBA). The target completion date is December 10, 2025.

### Project Milestones

1. Business problem statement
2. Exploratory data analysis
3. Modeling (logistic, decision tree, random forest, gradient boosting) with stratified 5-fold cross-validation
4. Presentation
5. Final report and monitoring plan
...
test <- read.csv(here::here("application_test.csv"))
train <- read.csv(here::here("application_train.csv"))
```
...
To start Model analysis it has been established that the current benchmark for Home Credit accuracy is 91.92% clients receiving a loan and successfully completing repayment, while 8.07% clients default on their repayment.   Through model analysis and variable tuning the goal is to achieve 94% successful client repayment.  

For modeling analyses we will use 4 types of models: logistic, tree model, random forest and gradient boosting. In some models you will see a 70:30 data partition split into training and test data. This is a very important step to withhold some of the data to be able to test our predictions once the model in question has been trained.  
...
# Plot ROC curve
plot(roc_obj, col = "blue", main = "ROC Curve - Random Forest (AUC: 0.651)")
...
# Plot ROC
plot(roc_obj, col = "blue", main = "ROC Curve - Gradient Boosting (AUC: 0.742)")
...
# Predictors used by the trained XGBoost model (order matters)
predictors <- c("EXT_SOURCE_3", "EXT_SOURCE_2", "export_source_1_imputed",
                "AGE_YEARS", "YEARS_EMPLOYED")

# Impute missing numeric values in test using *training* medians
# (train_data -> train, which already exists in the notebook)
test_imputed <- test
for (col in predictors) {
  if (is.numeric(test_imputed[[col]])) {
    med_val <- median(train[[col]], na.rm = TRUE)   # << minimal fix: use 'train'
    test_imputed[[col]][is.na(test_imputed[[col]])] <- med_val
  }
}

# ---- Minimal alignment to the model's feature set ----
feat <- if (!is.null(xgb_model$feature_names)) xgb_model$feature_names else predictors
# add any missing columns as zeros, drop extras, enforce order
missing <- setdiff(feat, colnames(test_imputed))
if (length(missing)) for (m in missing) test_imputed[[m]] <- 0
test_imputed <- test_imputed[, feat, drop = FALSE]

# Create DMatrix for XGBoost
test_matrix <- xgb.DMatrix(as.matrix(test_imputed))
